...
export const getCategories = async () => {
  const categories = await apiData.get(
    '/category2s?populate[image][populate]=*',
  );
...
export const getPopularPosts = async () => {
  const popularPosts = await apiData.get(
    '/post2s?populate[category_2][populate]=*&populate[author_2][populate]=*&populate[image][populate]=*&filters[isPopular][$eq]=true&pagination[page]=1&pagination[pageSize]=3',
  );
...
export const getRelatedPosts = async () => {
  const relatedPosts = await apiData.get(
    '/post2s?populate[category_2][populate]=*&populate[author_2][populate]=*&populate[image][populate]=*&filters[isPopular][$eq]=true&pagination[page]=1&pagination[pageSize]=4',
  );
...
export const getCategory = async documentId => {
  const category = await apiData.get(
    `/category2s/${documentId}?populate[image][populate]=*`,
  );
...
export const getAuthor = async authorDocumentId => {
  const author = await apiData.get(
    `/author2s/${authorDocumentId}?populate[avatar][populate]=*`,
  );
  return author.data;
};
...
export const getPostsByAuthor = async (
  ...
  const posts = await apiData.get(
    `/post2s?filters[author_2][documentId][$eq]=${authorDocumentId}&pagination[page]=${page}&pagination[pageSize]=${pageSize}&populate=*`,
  );
  return posts.data;
};
...
export const getSearchedPosts = async (query, page = 1, pageSize = 10) => {
  const posts = await apiData.get(
    `/post2s?filters[title][$containsi]=${query}&pagination[page]=${page}&pagination[pageSize]=${pageSize}&populate=*`,
  );
  return posts.data;
};
...
export const getNewPosts = async (page = 1, pageSize = 10) => {
  const posts = await apiData.get(
    `/post2s?pagination[page]=${page}&pagination[pageSize]=${pageSize}&populate=*`,
  );
  return posts.data;
};
