  ...
  return (
    <>
      <li
        ...
        className={`cht-forum__reply${depth > 0 ? ' cht-forum__reply--child' : ''}${
          isFresh ? ' cht-forum__reply--fresh' : ''
        }${isHighlighted ? ' cht-forum__reply--highlight' : ''}`}
      >
        <Identicon
          ...
          size={depth > 0 ? 36 : 44}
        />
        <div className="cht-forum__replyBody">
          <div className="cht-forum__replyMeta">
            <span className="cht-forum__replyAuthor">{comment.authorName}</span>
            {isOP && (
              <span className="cht-forum__opBadge" title="Themenstarter">
                Autor
              </span>
            )}
            {relative && (
              <>
                <span aria-hidden="true" style={{ color: 'var(--cht-muted)' }}>
                  ·
                </span>
                <time
                  className="cht-forum__replyTime"
          ...
          {parent && depth > 0 && (
            <p className="cht-forum__replyParent">
              ↩ Antwort an <strong>{parent.authorName}</strong>:{' '}
              {parent.body.length > 80
                ? `${parent.body.slice(0, 80)}…`
                : parent.body}
            </p>
          )}
          <p className="cht-forum__replyText">{linkify(comment.body)}</p>
          <div className="cht-forum__replyActions">
            <LikeButton
            ...
            {!isLocked && onReplyTo && (
              <button
                type="button"
                className="cht-forum__replyAction"
                onClick={() => onReplyTo(comment)}
                aria-label={`Auf ${comment.authorName} antworten`}
              >
            ...
            {onCopyLink && (
              <button
                ...
                aria-label="Link zu diesem Beitrag kopieren"
                title="Link kopieren"
              >
                <span aria-hidden="true">🔗</span>
                <span>Link</span>
  ...
  if (!comments.length) {
    return (
      <div className="cht-forum__empty">
        ...
        <p className="cht-forum__emptyTitle">Noch keine Antworten</p>
        <p className="cht-forum__emptyText">
          Sei der Erste, der seine Gedanken zu dieser Diskussion teilt.
        </p>
      </div>
    );
  }
